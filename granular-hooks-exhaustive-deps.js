module.exports = {
  meta: {
    ...
    docs: {
      description:
        'verifies the list of dependencies for granular Hooks like useGranularEffect and similar',
      recommended: true,
    },
    fixable: 'code',
    hasSuggestions: true,
  ...
  create(context) {
    ...
    function visitFunctionWithDependencies(
      ...
      const secondaryDeclaredDependencies = [];
      ...
      if (declaredDependenciesNode.type !== 'ArrayExpression') {
        ...
        reportProblem({
          ...
          message:
            ...
            'primary dependency list that is not an array literal. This means we ' +
        ...
        declaredDependenciesNode.elements.forEach(declaredDependencyNode => {
          ...
          if (declaredDependencyNode.type === 'SpreadElement') {
            reportProblem({
              ...
              message:
                ...
                "element in its primary dependency array. This means we can't " +
          ...
          if (useEventVariables.has(declaredDependencyNode)) {
            reportProblem({
              node: declaredDependencyNode,
              message:
                'Functions returned from `useEvent` must not be included in the primary dependency array. ' +
              ...
              if (declaredDependencyNode.type === 'Literal') {
                ...
                reportProblem({
                  ...
                  message:
                    ...
                    `complex expression in the primary dependency array. ` +
      ...
      if(secondaryDependenciesNode !== undefined) {
        if (secondaryDependenciesNode.type !== 'ArrayExpression') {
            // If the declared dependencies are not an array expression then we
            // can't verify that the user provided the correct dependencies. Tell
            // the user this in an error.
            reportProblem({
              node: secondaryDependenciesNode,
              message:
                `React Hook ${context.getSource(reactiveHook)} was passed a ` +
                'secondary dependency list that is not an array literal. This means we ' +
                "can't statically verify whether you've passed the correct " +
                'dependencies.',
            });
          } else {
            secondaryDependenciesNode.elements.forEach(secondaryDependencyNode => {
              // Skip elided elements.
              if (secondaryDependencyNode === null) {
                return;
              }
              // If we see a spread element then add a special warning.
              if (secondaryDependencyNode.type === 'SpreadElement') {
                reportProblem({
                  node: secondaryDependencyNode,
                  message:
                    `React Hook ${context.getSource(reactiveHook)} has a spread ` +
                    "element in its secondary dependency array. This means we can't " +
                    "statically verify whether you've passed the " +
                    'correct dependencies.',
                });
                return;
              }
              if (useEventVariables.has(secondaryDependencyNode)) {
                reportProblem({
                  node: secondaryDependencyNode,
                  message:
                    'Functions returned from `useEvent` must not be included in the secondary dependency array. ' +
                    `Remove \`${context.getSource(
                      secondaryDependencyNode,
                    )}\` from the list.`,
                  suggest: [
                    {
                      desc: `Remove the dependency \`${context.getSource(
                      ...
                      fix(fixer) {
                        return fixer.removeRange(secondaryDependencyNode.range);
                      },
                    },
                  ],
                });
              ...
              try {
                ...
                if (/Unsupported node type/.test(error.message)) {
                  if (secondaryDependencyNode.type === 'Literal') {
                    if (dependencies.has(secondaryDependencyNode.value)) {
                      reportProblem({
                        node: secondaryDependencyNode,
                        message:
                          `The ${secondaryDependencyNode.raw} literal is not a valid dependency ` +
                          `because it never changes. ` +
                          `Did you mean to include ${secondaryDependencyNode.value} in the array instead?`,
                      });
                    } else {
                      reportProblem({
                        node: secondaryDependencyNode,
                    ...
                    reportProblem({
                      ...
                      message:
                        `React Hook ${context.getSource(reactiveHook)} has a ` +
                        `complex expression in the secondary dependency array. ` +
                        'Extract it to a separate variable so it can be statically checked.',
                    });
                  }
              ...
              let maybeID = secondaryDependencyNode;
              while (
                maybeID.type === 'MemberExpression' ||
                maybeID.type === 'OptionalMemberExpression' ||
                maybeID.type === 'ChainExpression'
              ...
              // Add the dependency to our declared dependency map.
              secondaryDeclaredDependencies.push({
                key: declaredDependency,
                node: secondaryDependencyNode,
              });
      ...
      const allDeclaredDependencies = declaredDependencies.concat(secondaryDeclaredDependencies);
      ...
      const {
        ...
        dependencies,
        declaredDependencies: allDeclaredDependencies,
        stableDependencies,
      ...
      if (problemCount === 0) {
        ...
        constructions.forEach(
          ({construction, isUsedOutsideOfHook, depType}) => {
            const wrapperHook =
              depType === 'function' ? 'useGranularCallback' : 'useGranularMemo';
              ...
              suggest = [
                {
                  ...
                  fix(fixer) {
                    const [before, after] =
                      wrapperHook === 'useGranularMemo'
                        ? [`useGranularMemo(() => { return `, '; })']
                        : ['useGranularCallback(', ')'];
                    return [
    ...
    function visitCallExpression(node) {
      ...
      const primaryDependenciesNode = node.arguments[callbackIndex + 1];
      const secondaryDependenciesNode = node.arguments[callbackIndex + 2];
      ...
      // So no need to check for dependency inclusion.
      if (!primaryDependenciesNode && !isEffect) {
        // These are only used for optimization.
        if (
          reactiveHookName === 'useGranularMemo' ||
          reactiveHookName === 'useGranularCallback'
        ) {
      ...
      switch (callback.type) {
        ...
        case 'Identifier':
          ...
          if (
            primaryDependenciesNode.elements &&
            primaryDependenciesNode.elements.some(
...
function getReactiveHookCallbackIndex(calleeNode) {
  ...
  switch (node.name) {
    case 'useGranularEffect':
    case 'useGranularCallback':
    case 'useGranularMemo':
      return 0;
    default:
      return -1;
  }
